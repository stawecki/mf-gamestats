...
var string				ntcmd;
var string				ltcmd;
var string				autoplayL;
var int					autoplayN;
var bool				autoplayDetectEnd;
var bool				autoplayRepeatScores;
var int					autoplayRepeatScoresTime;
...
event bool KeyEvent( EInputKey Key, EInputAction Action, FLOAT Delta )
{
	...
	case IST_Press:
		...
		case EInputKey.IK_Escape:
			/* DEMO - Disable skipping past splash screens by exiting to frontend */
			/* FULLGAME - Condition no longer necessary */
			/*if (bLocked || Root.GetPlayerOwner().IsInState('GameEnded'))
				return true;*/
...
event Tick( float Delta )
{
	...
	// if autoplay queue started and cannot play next, look for an active GameClass to start observing end of video.
	if (autoplayN > 0 && autoplayDetectEnd == false) {
	  foreach Root.GetPlayerOwner().AllActors(class'GameReplicationInfo', GRI)
	   if (GRI.GameClass != "Rage.Inactive") {
	    autoplayDetectEnd = true;
	    Log("autoplay waiting for end");
	  }
	}

	// if autoplay queue started and waiting for demo to end, look for Rage.Inactive to trigger next video.
	if (autoplayN > 0 && autoplayDetectEnd == true) 
	{
		foreach Root.GetPlayerOwner().AllActors(class'GameReplicationInfo', GRI)
		{
			if (GRI.GameClass == "Rage.Inactive") {
				autoplayDetectEnd = false;
				autoplayN++;
				ntcmd = "DEMOPLAY "$autoplayL$autoplayN;
				Log("autoplay next -- "$ntcmd);
			}
	
			// also check for "repeat score" request after delay (scores take a while to settle):
			if (autoplayRepeatScores == true && GRI.ElapsedTime > autoplayRepeatScoresTime)
			{
			  autoplayRepeatScores = false;
			  autoplayRepeatScoresTime = 0;
			  Log("---begin-Scorecheck");
			    LogGameStatus();
			  Log("---end-Scorecheck");
			}
		}
...
{
    local PlayerReplicationInfo CPRI;
    local GameReplicationInfo GRI;

	if ( Viewport.Actor == None )		
           return;

    foreach Root.GetPlayerOwner().AllActors(class'PlayerReplicationInfo', CPRI)
      Log("---item-Score { pn: `"$CPRI.PlayerName$"`, s: `"$CPRI.Score$"`, d: `"$CPRI.Deaths$"`, tm: `"$CPRI.Team$"` }");

    foreach Root.GetPlayerOwner().AllActors(class'GameReplicationInfo', GRI)
    {
      Log("---attr-Game { gn: `"$GRI.GameName$"`, gc: `"$GRI.GameClass$"`, et: `"$GRI.ElapsedTime$"` }");
    }
}
...
event Message( PlayerReplicationInfo PRI, coerce string Msg, name N )
{
    ...
    Log("---begin-Event");
    LogGameStatus();
    Log("---attr-Msg { type: `"$N$"`, pn: `"$MsgPlayer[TopLine].PlayerName$"`, msg: `"$Msg$"` }");
    Log("---end-Event");

    if (N == 'Event')
      foreach Root.GetPlayerOwner().AllActors(class'GameReplicationInfo', GRI)
       {
         autoplayRepeatScoresTime = GRI.ElapsedTime + 2; // check after ~2 seconds
         autoplayRepeatScores = true;
       }

    // Fake "autoplay" command:
    if (inStr(Msg, "> autoplay") != -1)
    {
      autoplayDetectEnd = false; // detect end of video. When the demo starts playing, this is set to true.
      autoplayL = Right(Msg, 1); // autoplay Letter
      autoplayN = 1; // autoplay number
      ntcmd = "DEMOPLAY "$autoplayL$autoplayN$"?timebased";
      Log("autoplay start -- "$ntcmd);
    }
